/**
 * Модульный парсер для уникальных турниров
...
import { UniqueParseResultSchema } from "./schemas";

export async function parseUniqueHTML(html) {
...
export async function parseUniqueHTMLWithValidation(html) {
  const result = await parseUniqueHTML(html);
  
  // Валидация результата с помощью Zod
  const validationResult = UniqueParseResultSchema.safeParse(result);
