...
async function main() {
  const parsedFile = path.join(PARSED_DIR, '2023.json');
  const calendarFile = path.join(CALENDAR_DIR, '2023.json');
  ...
  console.log(`Processing 2023 (${parsedEvents.length} parsed events, ${calendarData.events.length} calendar events)...`);
  ...
  for (const parsedEvent of parsedEvents) {
    ...
    const month = getMonthFromDate(parsedEvent.date);
    const monthDir = path.join(COMPETITIONS_DIR, '2023', month);
