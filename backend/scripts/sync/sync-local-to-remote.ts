...
async function main() {
  ...
  for (const [key, parts] of Object.entries(sqlParts)) {
    const outPath = path.join(ROOT, `data/exports/sync-${key}.sql`);
    const content = parts.join("\n");
    await fs.writeFile(outPath, content);
    paths[key] = outPath;
    console.log(`  ${key}: ${(content.length / 1024 / 1024).toFixed(2)} MB → data/exports/sync-${key}.sql`);
  }

  const combinedPath = path.join(ROOT, "data/temp/sync-local-to-remote.sql");
  await fs.writeFile(
    combinedPath,
  ...
  if (push) {
    ...
    try {
      execSync(`npx wrangler d1 execute pc-db --remote --file=./data/migrations/migrate-remote-schema.sql -y`, {
        cwd: ROOT,
        stdio: "inherit",
      });
    } catch {
    ...
    for (const key of ["events", "dogs", "results"]) {
      ...
      execSync(`npx wrangler d1 execute pc-db --remote --file=./data/exports/sync-${key}.sql -y`, {
        cwd: ROOT,
        stdio: "inherit",
      });
    }
