...
// Режим валидации: 'warn' для разработки, 'error' для CI
const VALIDATION_MODE: ValidationMode = process.env.SCHEMA_VALIDATION_MODE === 'error' ? 'error' : 'warn';
...
export function syncSqliteToV1(db: Database.Database): { competitions: number; dogs: number } {
  ...
  for (const rawEvent of events) {
    ...
    const competitionValidation = validateData(
      CompetitionV1Schema,
      competitionPayload,
      VALIDATION_MODE,
      `competition-${eventId}`
    ...
    if (!competitionValidation.valid) {
      throw new Error(`Schema validation failed for competition-${eventId}:\n${competitionValidation.errors.join('\n')}`);
    }

    writeJson(path.join(V1, rel), competitionPayload);
    competitionCount += 1;
  }
  ...
  for (const d of dogs) {
    ...
    // Валидация перед записью
    const dogValidation = validateData(
      DogV1Schema,
      dogPayload,
      VALIDATION_MODE,
      `dog-${id}`
    ...
    if (!dogValidation.valid) {
      throw new Error(`Schema validation failed for dog-${id}:\n${dogValidation.errors.join('\n')}`);
    }

    writeJson(path.join(V1, `dogs/by-id/${id}.json`), dogPayload);
    writeJson(path.join(V1, `dogs/by-key/${dk}.json`), dogPayload);
  }
