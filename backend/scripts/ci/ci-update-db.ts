...
async function main() {
  ...
  run("npx tsx backend/scripts/update/update-current-year.ts");
  run(`npx tsx backend/scripts/load/load-events.ts ${eventsFile}`);
  ...
  run(`npx tsx backend/scripts/load/load-results.ts ${eventsFile}`);
