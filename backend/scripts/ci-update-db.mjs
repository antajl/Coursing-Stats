...
async function main() {
  ...
  const eventsFile = "data/events-current.json";
  const eventsSql = "data/load-events.sql";
  const resultsSql = "data/load-results.sql";

  run("node backend/scripts/update-current-year.mjs");
  run(`node backend/scripts/load-events.mjs ${eventsFile}`);
  ...
  run(`node backend/scripts/load-results.mjs ${eventsFile}`);
