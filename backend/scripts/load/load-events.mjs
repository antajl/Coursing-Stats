...
async function loadEvents() {
  ...
  for (const event of events) {
    ...
    // Используем составной ключ для конфликта (работает и для null results_url)
    const sql = `
INSERT INTO events (
...
ON CONFLICT(date_start, title, location, event_type) DO UPDATE SET
  ...
  console.log("  sqlite3 procoursing.db < data/imports/load-events.sql");
  console.log("\nCloudflare D1 (local):");
  console.log("  wrangler d1 execute procoursing-db --local --file=./data/imports/load-events.sql");
  console.log("\nCloudflare D1 (production):");
  console.log("  wrangler d1 execute procoursing-db --file=./data/imports/load-events.sql");
}
