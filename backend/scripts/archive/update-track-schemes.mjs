...
/**
 * Update track_schemes field for all coursing events
 * Generates SQL UPDATE statements for manual execution
 * 
 * Usage:
 * node scripts/update-track-schemes.mjs
 */

async function updateTrackSchemes() {
  console.log('Starting track schemes update...');
  ...
  for (const event of events) {
    try {
      ...
      // Generate SQL UPDATE statement if track schemes found
      if (parsedData.track_schemes && parsedData.track_schemes.length > 0) {
        const trackSchemesJson = JSON.stringify(parsedData.track_schemes).replace(/'/g, "''");
        const sql = `UPDATE events SET track_schemes = '${trackSchemesJson}' WHERE id = ${event.id};`;
        sqlStatements.push(sql);
        console.log(`  Found ${parsedData.track_schemes.length} track schemes`);
        found++;
      } else {
        console.log(`  No track schemes found in HTML`);
      }
  ...
  fs.writeFileSync('./data/update-track-schemes.sql', sqlStatements.join('\n'));
  
  console.log(`\n✓ Track schemes update completed`);
  console.log(`  Events processed: ${processed}/${events.length}`);
  console.log(`  Events with track schemes: ${found}`);
  console.log(`  Errors: ${errors}`);
  console.log(`\nSQL statements saved to data/update-track-schemes.sql`);
  console.log(`Run: wrangler d1 execute pc-db --local --file=./data/update-track-schemes.sql`);
}

updateTrackSchemes().catch(console.error);
