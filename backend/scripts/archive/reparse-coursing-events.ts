...
import { loadDogIndexFromD1, lookupDogIdFromIndex } from "../../lib/dog-lookup";
  ...
  try {
    ...
    for (const event of events) {
      try {
        ...
        console.log(`  Found ${parsedData.results.length} results`);
        
        // Update event with judges info if available
        if (parsedData.judges) {
          await db.prepare(`
            UPDATE events 
            SET judges = ?
            WHERE id = ?
          `).bind(parsedData.judges, event.id).run();
          console.log(`  Updated judges: ${parsedData.judges}`);
        }
