...
function parseArgs(argv: string[]) {
  ...
  for (const arg of argv) {
    ...
    if (arg === '--auto-process') autoProcess = true
  }
  return { year, limit, concurrency, pauseMs, autoProcess }
}
...
async function main() {
  const { year, limit, concurrency, pauseMs, autoProcess } = parseArgs(process.argv.slice(2))
  ...
  // Auto-process downloaded PDFs if requested
  if (autoProcess && downloaded > 0) {
    console.log(`\n🔄 Auto-processing ${downloaded} new PDF files...`)
    await runAutoProcess(year)
  }
}

async function runAutoProcess(year: string) {
  const { exec } = await import('node:child_process')
  return new Promise<void>((resolve, reject) => {
    const process = exec('npm run auto-process-pdf', { cwd: ROOT }, (error, stdout, stderr) => {
      if (error) {
