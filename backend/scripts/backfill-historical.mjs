...
const HISTORICAL_YEARS = [2015, 2016, 2017, 2018, 2019, 2020, 2021, 2022, 2023, 2024, 2025, 2026];
...
async function main() {
  console.log("Бэкафилл исторических годов (2015-2025)...");
  console.log("Это займет время - обрабатывается каждый год отдельно с паузами.");
  ...
  for (const year of HISTORICAL_YEARS) {
    ...
    await sleep(2000); // вежливая пауза между годами
  }

  await fs.writeFile("data/events-historical.json", JSON.stringify(all, null, 2), "utf-8");
  console.log(`\nГотово. Всего событий: ${all.length}`);
  console.log("Записано в data/events-historical.json");
  console.log("\nДалее:");
  console.log("1. node backend/scripts/load-events.mjs data/events-historical.json");
  console.log("2. node backend/scripts/load-results.mjs");
