export async function handleJudges(path, url, db, corsHeaders) {
  // GET /api/judges
  if (path === '/api/judges') {
    ...
    const judgeStats = {}; // Ключ: фамилия судья
    ...
    for (const [name, stats] of Object.entries(judgeStats)) {
      const validScores = stats.scores.filter(s => s !== null && !isNaN(s));
  ...
  if (path.match(/^\/api\/judges\/[^\/]+\/details$/)) {
    const judgeName = decodeURIComponent(path.split('/')[3]);
    const breed = url.searchParams.get('breed') || '';
    const discipline = url.searchParams.get('discipline') || '';
    ...
    const criteriaStats = { 0: [], 1: [], 2: [], 3: [], 4: [] }; // 5 категорий оценок
    const dogTotalScores = {}; // Суммарные оценки для каждой собаки
    ...
    for (const row of results) {
      try {
        ...
        if (judgeNum > 0) {
          for (const heat of heats) {
            if (heat.judges && Array.isArray(heat.judges)) {
              const judge = heat.judges.find(j => j.judge_number === judgeNum);
              if (judge && judge.scores && Array.isArray(judge.scores)) {
                const validScores = judge.scores.filter(s => s !== null && !isNaN(s));
    ...
    for (const [breed, stats] of Object.entries(breedStats)) {
      ...
      const dogsArray = Object.values(dogStats).map(dog => ({
        name: dog.name,
        name_ru: dog.name_ru,
              ...
              for (const heat of heats) {
                if (heat.judges && Array.isArray(heat.judges)) {
                  ...
                  if (judge && judge.scores && Array.isArray(judge.scores)) {
                    ...
                    const dogData = dogsArray.find(d => 
    ...
    breedData.sort((a, b) => b.count - a.count);
    
    const overallAvg = allScores.length > 0 
      ? allScores.reduce((a, b) => a + b, 0) / allScores.length 
      : null;
    
    recentEvaluations.sort((a, b) => new Date(b.date) - new Date(a.date));
    ...
    const dogScoresData = Object.values(dogTotalScores).map(dog => ({
    ...
    dogScoresData.sort((a, b) => b.total_score - a.total_score);
...
function parseJudgeNames(judgesString) {
