...
-- Modified: removed total_score IS NOT NULL requirement to handle parsing edge cases
-- Modified: added COALESCE fallback for total_score to compute from raw_scores_json
...
SELECT
  ...
  e.year,
  MAX(
    COALESCE(r.total_score,
      (SELECT SUM(CAST(json_extract(h.value, '$.total') AS REAL))
       FROM json_each(json_extract(r.raw_scores_json, '$.heats')) AS h
       WHERE json_extract(h.value, '$.total') IS NOT NULL)
    )
  ) AS best_score,
  COUNT(*) AS total_starts,
  MAX(
