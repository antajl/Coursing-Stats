...
describe('exhibition-catalog', () => {
  ...
  it.skipIf(!fs.existsSync(ruFixturePath))('builds Russian catalog from ru-RU fixture', () => {
  ...
  it.skipIf(!fs.existsSync(ruFixturePath))('maps English result rows to Russian catalog entries', () => {
    const raw = JSON.parse(fs.readFileSync(ruFixturePath, 'utf-8')) as {
      catalogSample: ExhibitionCatalogResponse
    }
    const catalog = buildBreedCatalog(raw.catalogSample)
  ...
  it.skipIf(!fs.existsSync(enFixturePath) || !fs.existsSync(ruFixturePath))('builds catalog from en fixture with dom group names', () => {
