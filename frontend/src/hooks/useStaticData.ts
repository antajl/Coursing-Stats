...
export function useTopPlacement(year: string) {
  ...
  return useQuery({
    ...
    queryFn: () => fetchStaticData<TopItemsResponse>(`/data/v1/indexes/${fileName}`),
    staleTime: STALE_MS,
  });
}
...
export function useTopElo(year: string) {
  const fileName = topIndexFileName('top-elo', year);
  return useQuery({
    queryKey: ['topElo', year || 'all'],
    queryFn: () => fetchStaticData<TopItemsResponse>(`/data/v1/indexes/${fileName}`),
    staleTime: STALE_MS,
  });
}
...
export function useCompetingBreeds() {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return { success: true as const, data: { breeds, count: breeds.length, dogIndex: result.data } };
    },
    staleTime: STALE_LONG_MS,
  });
}
...
export function useShowJudges(options?: { enabled?: boolean }) {
  return useQuery({
    queryKey: ['showJudges'],
...
export function useShowJudgesPage0() {
  return useQuery({
