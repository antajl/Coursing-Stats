...
export default function FilterSelect({
  ...
  const selectBase =
    'h-8 w-full min-w-0 max-w-full px-3 bg-white border-[1.5px] border-old-money-300 rounded-lg text-xs font-medium text-charcoal-700 focus:ring-2 focus:ring-gold-400 focus:border-transparent transition-colors'
  ...
  return (
    <div className={className}>
      {label && (
        <label
          htmlFor={id}
          className="block text-sm font-semibold text-old-money-700 mb-2"
        >
          {label}
        </label>
      )}
