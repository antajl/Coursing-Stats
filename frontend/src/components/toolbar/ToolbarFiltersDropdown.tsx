  ...
  return (
    <div className="relative shrink-0" ref={ref}>
      ...
      >
        {label}
        {showBadge ? (
          <span className="inline-flex h-4 min-w-4 items-center justify-center rounded-full bg-charcoal-900/15 px-1 text-[10px] font-bold tabular-nums text-charcoal-900">
      ...
      {open ? (
        <>
          ...
          >
            ...
            <div className="shrink-0 border-t border-old-money-200/80 bg-cream-50/95 px-4 pb-[max(0.75rem,env(safe-area-inset-bottom))] pt-3 backdrop-blur-sm">
              <div className="flex gap-2">
                <button
                  ...
                  className="flex-1 rounded-lg border border-old-money-200 bg-white px-3 py-2.5 text-xs font-medium text-charcoal-700 transition-colors hover:bg-old-money-100"
                >
                  Сбросить
                </button>
                <button
                  type="button"
                  onClick={() => setOpenState(false)}
                  className="flex-1 rounded-lg bg-camel-600 px-3 py-2.5 text-xs font-semibold text-charcoal-900 transition-colors hover:bg-camel-500"
                >
                  Готово
                </button>
              </div>
