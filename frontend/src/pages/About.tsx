...
function ContactLink({
  ...
  return (
    <a
      ...
      className="flex items-center gap-3 rounded-lg border border-old-money-200 bg-old-money-50/60 px-4 py-3 transition-colors hover:border-camel-300 hover:bg-camel-50/80"
    >
      <span className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-lg border border-old-money-200 bg-white">
        <Icon className="h-5 w-5 text-camel-700" aria-hidden />
      </span>
      <span className="min-w-0">
        <span className="block text-[10px] font-semibold uppercase tracking-wide text-old-money-500">
          {label}
        </span>
        <span className="block font-medium text-camel-800">{value}</span>
      </span>
    </a>
  )
}
...
function DiscordContact() {
  ...
  return (
    <button
      type="button"
      onClick={copy}
      className="flex w-full items-center gap-3 rounded-lg border border-old-money-200 bg-old-money-50/60 px-4 py-3 text-left transition-colors hover:border-camel-300 hover:bg-camel-50/80"
      title="Скопировать ник Discord"
    >
      <span className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-lg border border-old-money-200 bg-white">
        <MessageCircle className="h-5 w-5 text-camel-700" aria-hidden />
      </span>
      <span className="min-w-0 flex-1">
        <span className="block text-[10px] font-semibold uppercase tracking-wide text-old-money-500">
          Discord
        </span>
        <span className="block font-medium text-camel-800">antajl</span>
        <span className="mt-0.5 block text-[11px] text-old-money-500">
          {copied ? 'Ник скопирован' : 'Нажмите, чтобы скопировать ник'}
        </span>
      </span>
      {copied ? (
        <Check className="h-4 w-4 flex-shrink-0 text-camel-600" aria-hidden />
      ) : (
...
const AUDIENCE = [
  {
    ...
    text: 'Сводка по собаке: участия, оценки, медали, выставочные награды и рекорды Донино — в одном профиле со ссылками на исходные протоколы.',
  },
  {
