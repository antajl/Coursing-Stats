  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-old-money-50 to-gold-50 p-6">
      <div className="max-w-4xl mx-auto">
        ...
        {/* Результаты */}
        <div className="bg-old-money-50 rounded-2xl shadow-2xl border border-gold-200 p-6">
          <h2 className="text-xl font-serif font-bold text-gold-700 mb-4">Результаты</h2>
                          ...
                          return (
                            <details key={`${result.dog_id}-${idx}`} className="group">
                              <summary className="list-none cursor-pointer">
                                <div className="flex items-center gap-4 p-3 rounded-lg hover:bg-old-money-100 transition-colors">
                                  {/* Medal/Place */}
                                  <div className="flex-shrink-0">
                                    {result.status === 'disqualified' ? (
                                      ...
                                      <div className="w-10 h-10 rounded-full bg-gray-300 border-2 border-gray-400 flex items-center justify-center text-gray-700 font-bold text-sm">
                                        {result.placement}
                                      </div>
                                    ) : result.placement === 3 ? (
                                      <div className="w-10 h-10 rounded-full bg-orange-400 border-2 border-orange-500 flex items-center justify-center text-white font-bold text-sm">
                                        {result.placement}
                                      </div>
                                    ) : result.placement ? (
                                      <div className="w-10 h-10 rounded-full bg-old-money-200 border border-old-money-300 flex items-center justify-center text-old-money-700 font-bold text-sm">
                                        {result.placement}
                                      </div>
                                    ) : null}
                                  </div>
                                  ...
                                  <div className="flex-1 min-w-0">
                                    <div className="flex items-baseline gap-2">
                                      <span className="font-serif font-medium text-old-money-800 truncate">
                                        {result.name_ru || result.name_lat}
                                      </span>
                                      {result.name_ru && result.name_lat && result.name_ru !== result.name_lat && (
                                        <span className="font-serif text-sm text-old-money-600 truncate">
                                          / {result.name_lat}
                                        </span>
                                      )}
                                    </div>
                                    ...
                                    {!isDisqualified && result.qualification && (
                                      <div className="flex gap-1.5 mt-1 flex-wrap">
                                        {result.qualification.split(',').map((title, i) => {
                                          ...
                                          return (
                                            <span key={i} className={`inline-block px-2 py-0.5 rounded-full text-xs font-medium ${
                                  ...
                                  <div className="flex-shrink-0 text-right">
                                    {result.total_score ? (
                                      <>
                                        <div className="text-lg font-bold text-gold-700">{result.total_score}</div>
                                        <div className="text-xs text-old-money-600">баллов</div>
                                      </>
                                    ) : result.status_reason ? (
                                      <div className="text-sm text-red-600 italic">{result.status_reason}</div>
                                    ) : null}
                                  </div>
                              ...
                              <div className="mt-2 ml-14 mr-20 overflow-x-auto">
                                {rawScores && rawScores.heats && rawScores.heats.length > 0 && (
                                  <table className="w-full text-xs min-w-[600px]">
                                    ...
                                    <tbody>
                                      {rawScores.heats[0].judges.map((judge, judgeIdx) => {
                                        const judgeLabel = judge.judge_number === 1 ? 'Главный судья' : 'Судья';
