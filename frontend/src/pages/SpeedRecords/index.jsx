...
function SpeedRecords() {
  ...
  // Состояние для coursing records
  const [coursingRecords, setCoursingRecords] = useState([]);
  const [coursingLoading, setCoursingLoading] = useState(true);
  ...
  async function loadCoursingRecords() {
    setCoursingLoading(true);
    try {
      const result = await api.getCoursingRecords('', '', 1000, '');
      if (result.success) {
        ...
        // Группируем по кличке и породе, оставляем только лучший результат
        const bestRecords = [];
        const seenKeys = new Set();
        
        // Сортируем по времени ASC (лучшее время первым)
        const sortedRecords = recordsWithHistory.sort((a, b) => a.time_seconds - b.time_seconds);
        ...
        setCoursingRecords(bestRecords);
      }
    } catch (err) {
      console.error('Error fetching coursing records:', err);
    } finally {
      setCoursingLoading(false);
    }
  }
  ...
  async function loadRecords() {
    ...
    if (result.success) {
      ...
      // Группируем по кличке, породе и полу, оставляем только лучший результат
      const bestRecords = [];
      const seenKeys = new Set();
      
      // Сортируем по скорости DESC (лучший результат первым)
      const sortedRecords = recordsWithHistory.sort((a, b) => b.speed_km_h - a.speed_km_h);
