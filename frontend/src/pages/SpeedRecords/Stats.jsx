...
function SpeedRecordsStats() {
  ...
  const breedStats = breeds.map(breed => {
    ...
    return { breed, count: breedRecords.length, avgSpeed, maxSpeed, avgTime350, bestTime350 };
  ...
  const sexStats = sexes.map(sex => {
    ...
    const avgTime350 = avgSpeed > 0 ? (1260 / avgSpeed).toFixed(2) : '-';
    const bestTime350 = maxSpeed > 0 ? (1260 / maxSpeed).toFixed(2) : '-';
    return { sex: sex === 'С' ? 'Сука' : 'Кабель', count: sexRecords.length, avgSpeed, maxSpeed, avgTime350, bestTime350 };
  });
  ...
  const yearStats = years.map(year => {
    ...
    const avgTime350 = avgSpeed > 0 ? (1260 / avgSpeed).toFixed(2) : '-';
    const bestTime350 = maxSpeed > 0 ? (1260 / maxSpeed).toFixed(2) : '-';
    return { year, count: yearRecords.length, avgSpeed, maxSpeed, avgTime350, bestTime350 };
          ...
          <table className="w-full min-w-[600px]">
            <thead>
              <tr className="border-b-2 border-cream-300">
                ...
                <th className="px-4 py-3 text-center font-semibold text-charcoal-900">Время за 350м (ср)</th>
                <th className="px-4 py-3 text-center font-semibold text-charcoal-900">Время за 350м (лучшее)</th>
              </tr>
            </thead>
            <tbody>
            ...
            return (
              <div key={idx} className="bg-cream-50 rounded-lg p-4 border border-cream-200 cursor-pointer hover:bg-cream-100 transition-colors" onClick={() => setExpandedSection(isExpanded ? null : `sex-${stat.sex}`)}>
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex justify-between">
                    <span className="text-old-money-600">Время за 350м (ср):</span>
                    <span className="font-semibold text-charcoal-900">{stat.avgTime350} сек</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-old-money-600">Время за 350м (лучшее):</span>
                    <span className="font-bold text-camel-700">{stat.bestTime350} сек</span>
                  </div>
                </div>
                {isExpanded && (
