      ...
      <div className="flex flex-wrap gap-2">
        {items.slice(0, 8).map((item) => (
          <Link
            ...
            className="inline-flex items-center gap-2 rounded-full border border-charcoal-200 bg-white/70 px-3 py-1.5 text-sm text-charcoal-800 hover:border-camel-500 hover:text-camel-800 transition-colors"
          >
            <span>{item.breedDisplay}</span>
            {item.count > 1 && (
              <span className="tabular-nums text-charcoal-500">{item.count}</span>
            )}
          </Link>
        ))}
      </div>
