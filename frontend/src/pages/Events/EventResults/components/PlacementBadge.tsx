  ...
  if (result.status === 'disqualified') {
    return (
      <HoverTooltip label="Отстранение" placement="top" variant="site" delayMs={0} portal>
        <div className="w-6 h-6 md:w-7 md:h-7 rounded-full bg-red-100 border-2 border-red-500 flex items-center justify-center cursor-help">
  ...
  if (result.status === 'dns') {
    return (
      <HoverTooltip label="Неявка" placement="top" variant="site" delayMs={0} portal>
        <div className="w-6 h-6 md:w-7 md:h-7 rounded-full bg-gray-100 border-2 border-gray-400 flex items-center justify-center cursor-help">
  ...
  if (result.placement) {
    return (
      <HoverTooltip label={`${result.placement} место`} placement="top" variant="site" delayMs={0} portal>
        <div className="w-6 h-6 md:w-7 md:h-7 rounded-full bg-old-money-200 border border-old-money-300 flex items-center justify-center text-old-money-700 font-bold text-[10px] md:text-xs cursor-help">
          {result.placement}
        </div>
      </HoverTooltip>
    )
