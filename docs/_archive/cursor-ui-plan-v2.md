...
| v2.1-B | Рекорды Донино: двухколоночный hub (Записи \| Статистика) | ✅ |
...
### v2.1-B. Рекорды Донино — hub ✅ (обновлено 2026-07-06)

**Было (v2.1):** сегменты «Замер» | «Бега 350 м» в `SpeedRecords/index.tsx`.

**Стало:** одна страница, **две колонки**; переключатель **Записи | Статистика** (`DoninoPageToolbar` + `ViewToggle`). См. `SPEED-RECORDS.md`, `DEVELOPMENT.md` § SpeedRecords.
...
Долгосрочное (Shared TabBar) — **не сюда**, см. `FUTURE-PLANS.md`.

**Календарь → `PageToolbar`:** ✅ выполнено (2026-07-06), `Events/index.tsx`.
