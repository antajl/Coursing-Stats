...
dogs/by-id/{id}.json  (все ~2034 id)
        ...
        └─ build-derived-indexes
              ...
              • top-score-*: attachScoreMetrics → avg_judge_score, judge_eval_count, rating_score; сортировка по rating_score
```

### Индекс `top-score-*` (курсинг + БЗМП, вкладка «очки»)

| Поле | Смысл |
|------|--------|
| `best_score` | Макс. сумма протокола (`grand_total`) — **только справка**, не порядок в списке |
| `best_judge_score` | Лучшая оценка одного судьи за пробу |
| `avg_judge_score` | Средняя по всем оценкам судей (все пробы) |
| `judge_eval_count` | Число отдельных оценок судей (для сглаживания) |
| `rating_score` | **Индекс CS** — ключ сортировки на сайте |

Формула и константы: `backend/lib/rating/coursing-rating-score.ts`. Пересборка: `npm run build-all-data`.

**Полная формула** (μ — `avg_judge_score`, n — `judge_eval_count`, B — `best_judge_score`, S — `total_starts`):

```
μ̃ = (μ × n + 85 × 12) / (n + 12)          # сглаженная средняя
P   = 0,15 × min(B − μ̃, 4)   если B > μ̃, иначе 0   # бонус пика (макс. +0,6)
E   = min(2, 0,5 × log₂(S + 1))                 # бонус стартов (макс. +2)
...
**Пример** (СТАНГЕРС ЛАНД ИНГРИД ЭЛЕГАНТ): μ=87, n=64, B=97, S=16 → μ̃=86,68, P=0,60, E=2,00 → **CS=89,28**.

**Контраст** (1 старт, μ=96, n=4, B=98, S=1): μ̃=87,75, P=0,60, E=0,50 → **CS≈88,85** — ниже стабильной карьеры (89,28).
