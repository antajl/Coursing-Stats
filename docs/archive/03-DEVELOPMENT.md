...
## Процедура после изменений парсеров результатов

Если вы изменили логику парсинга результатов (coursing, BZMP или racing), необходимо:

1. **Изменить код парсера**
   - Файлы: `backend/parsers/parse-results-coursing.mjs`, `backend/parsers/parse-results-bzmp.mjs`, `backend/parsers/parse-results-racing.mjs`
   
2. **Перезапустить парсинг**
   ```bash
   cd backend
   node scripts/reparse-coursing-events.mjs  # для курсинга
   node scripts/reparse-bzmp-events.mjs       # для БЗМП
   node scripts/reparse-racing-events.mjs     # для рейсинга
   ```
   Это создаст SQL файл в `data/reparse-coursing.sql`, `data/reparse-bzmp.sql` или `data/reparse-racing.sql`

3. **Обновить базу данных**
   ```bash
   wrangler d1 execute pc-db --remote --file=./data/reparse-coursing.sql
   wrangler d1 execute pc-db --remote --file=./data/reparse-bzmp.sql
   wrangler d1 execute pc-db --remote --file=./data/reparse-racing.sql
   ```
...
## Парсинг информации о судьях

### Проблема
Изначально парсеры искали информацию о судьях в тексте ДО первой таблицы, но на страницах procoursing.ru судьи находятся ВНУТРИ таблицы в строке с `colspan=25` (для курсинга и БЗМП) или `colspan=18` (для рейсинга).

### Решение
Добавлен поиск судей в строках таблицы с соответствующим colspan:
- Курсинг и БЗМП: `colspan=25`
- Рейсинг: `colspan=18`

Парсер теперь:
1. Ищет строки с нужным colspan
2. Проверяет наличие текста "Судьи:" или "Судьи"
3. Извлекает имена судей из следующей строки или из той же ячейки
4. Поддерживает 1, 2 или 3 судей
5. Использует гибкие паттерны для разных форматов имен

### Форматы
- Многострочный: "Судьи:\nГлавный судья - Иванова Г.С., судья - Богаченко В.В."
- Однострочный: "Главный судья - Лукина Д.М., судья - Гродинская Т.Л."
- Один судья: "Главный судья - Карелина Н"
...
### Обзор
Модуль статистики судей позволяет просматривать детальную информацию о работе судей на соревнованиях по курсингу, БЗМП и бегам.

### Техническая реализация

**Фронтенд:**
- `frontend/src/pages/Judges.jsx` — список судей с фильтрами и сортировкой
- `frontend/src/pages/JudgeDetail.jsx` — детальная страница судьи
- `frontend/src/App.jsx` — маршрут `/judges/:judgeId`

**Бэкенд:**
- `backend/src/routes/judges.js` — API endpoints:
  - `GET /api/judges` — список судей с фильтрами
  - `GET /api/judges/:id/details` — детальная статистика судьи
  ...
  "breed_stats": [
    {
      ...
      "dogs": [
        {
          "name": "ЭМУЛЬ ДЭ ГЕПАРД ГЕЛИЛА АЛЬ РАВДА",
          "name_ru": "Гелила",
          "avg_score": 4.7,
          "total_evaluations": 5,
          "scores_by_criteria": {
          ...
          "events": ["Чемпионат 2026", "Кубок 2025"]
        }
      ]
    }
  ],
...
**Список судей (Judges.jsx):**
- Фильтрация по породе и дисциплине
- Сортировка по всем колонкам (клик на заголовок)
- Колонки: Судья, Оцениваний, Всего оценок, Средняя оценка, Пород, Дисциплин
- Клик на строку → переход на детальную страницу судьи

**Детальная страница судьи (JudgeDetail.jsx):**
- Общая статистика: всего оценок, средняя оценка
- Статистика по породам (раскрывающиеся строки):
  - Порода, Оцениваний, Оценок, Средняя, Мин, Макс
  - При клике → список собак с их статистикой
  - Для каждой собаки: имя, оцениваний, средняя
  - При клике на собаку → оценки по 5 категориям
- Статистика по критериям:
  - Критерий, Оцениваний, Оценок, Средняя, Мин, Макс
  - Сортировка по всем колонкам

### Особенности реализации

**Подсчет оцениваний:**
- Одно оценивание = 5 категорий оценок (Манёвренность, Резвость, Выносливость, Преследование, Энтузиазм)
- Количество оцениваний = количество оценок / 5
- Используется Math.round() для округления

**Фильтр по породам:**
- API возвращает список доступных пород (`available_breeds`)
- Фронтенд заполняет dropdown из этого списка
- При изменении фильтра данные перезагружаются

**Сортировка:**
- Каждая таблица имеет независимую сортировку
- Стрелки (↑/↓) показывают текущее поле и направление
- При повторном клике направление меняется

**Имена собак:**
- Русское имя добавляется в скобках только если не содержится в латинском
- Проверка: `!dog.name.includes(dog.name_ru)`

### История изменений

**2026-06-27:**
- Создана отдельная страница JudgeDetail.jsx для детальной статистики
- Добавлен маршрут `/judges/:judgeId` в App.jsx
- Изменен Judges.jsx для перехода на отдельную страницу вместо inline-раскрытия
- Добавлен фильтр по породам с динамическим списком из API
- Добавлено раскрывающееся меню пород с детальной статистикой собак
- Добавлено раскрытие оценок по категориям для каждой собаки
- Изменен подсчет: оценки → оценивания (деление на 5)
- Убрано дублирование имен собак
- Убраны лишние фразы ("Собаки:", "Оценки по категориям:")
- Удалены блоки "Последние оценки" и "Топ собак"
- Добавлены колонки "Оцениваний" и "Оценок" во все таблицы
