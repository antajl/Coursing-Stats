...
### Когда D1 был нужен (legacy)

D1 использовался для автоматизации и CI/CD. Многие команды ниже **могут отсутствовать** в текущем `package.json` — сверяйтесь с кодом.
...
**Таблица `judges`:** на remote **отсутствует**; статистика судей агрегируется из `events.judges` и `results.raw_scores`. Отдельная materialized-таблица — не реализована (runtime судьи — из `indexes/`).
...
> **Связанные файлы:** `../14-PARSING-RULES.md` (детали HTML), `../11-DEVELOPMENT.md` (скрипты).
...
Парсеры: `backend/parsers/{coursing,bzmp,racing}/index.ts` через `reparse-by-year.ts`.  
