...
1. **Всегда читай 00-PROJECT-STATUS.md первым** — там полный контекст проекта
...
1. **Синтетические тесты:** `npx tsx backend/scripts/test/test-parser.ts`
...
- `scripts/parse-results-coursing.ts` — есть рабочий тест (`scripts/test/test-parser.ts`), но он на синтетических данных. ОБЯЗАТЕЛЬНО прогони на 5-10 реальных страницах разных лет, прежде чем доверять результатам массово
- БЗМП и бега (Racing) — отдельные форматы страниц, парсер для них уже написан. Не предполагай, что формат курсинга подойдёт

## Рабочий процесс

1. Перед любым изменением парсера — прогони `npx tsx backend/scripts/test/test-parser.ts`. После изменения — прогони снова, тест должен остаться зелёным (или обнови тест, если изменилась ожидаемая структура, и объясни почему)
