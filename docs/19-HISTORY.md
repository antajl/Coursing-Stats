...
## 2026-07-11 — UI улучшения главной страницы

### Hero CTA кнопки (Hero.tsx)
- **Удалена:** кнопка "Судьи" (избыточна - доступна через Nav → Статистика → Судьи)
- **Переименована:** "Курсинг Донино" → "Рекорды Донино" для консистентности с навигацией
- **Изменён стиль:** btn-ghost → btn-secondary для лучшей визуальной иерархии
- **Результат:** Две кнопки в hero: "Статистика" (primary) и "Рекорды Донино" (secondary)

### Заголовок H1 (Hero.tsx)
- **Было:** "Статистика курсинга и бегов борзых в одном месте"
- **Стало:** "Статистика курсинга и бегов борзых"
- **Причина:** Убрана шаблонная концовка "в одном месте", дублирующая eyebrow

### HomeEventRow компактный режим (HomeEventRow.tsx)
- **Проблема:** Компактный режим показывал только числа дней без месяца, путал пользователей
- **Решение:** Показывать месяц вместо дня недели в компактном режиме (напр. "авг", "сен")
- **Изменение:** `dateParts.metaLine` → `dateParts.month` для compact mode
- **Результат:** Пользователи теперь видят месяц без клика на procoursing.ru

### Анимации главной страницы (Home.tsx)
- **Удалены:** Индивидуальные animation hooks для каждой секции
- **Добавлена:** Система двух волн анимации
  - Волна 1: Hero block (`.hero-dashboard [data-rise]`, stagger 0.055s, duration 0.36s)
  - Волна 2: Stats + секции ниже (`.stats-strip-wrap [data-rise], .home-below-fold [data-rise]`, delay 0.08s, stagger 0.05s, duration 0.36s)
- **Удалена:** Специальная логика анимации подиума (riseInPodium/fadeInPodium)
- **Удалена:** Внутренняя анимация компонента Hero
- **Добавлен:** Обёртка `home-below-fold` для группировки секций
- **Результат:** Упрощённая система анимации, ~0.5s вместо ~1s
...
- `build-all-data` + CI копирует в `frontend/public/data/v1/` (generated, not in git)
