...
- **Нормализация total_score:** ВСЕГДА делить на количество судей (grand_total / judge_count) для сравнимости. Сырая сумма не используется — она зависит от числа судей (1=200 max, 2=400 max, 3=600 max)
...
**Нормализация:** Для сравнимости результатов между событиями с разным количеством судей, total_score всегда делится на количество судей (grand_total / judge_count).
