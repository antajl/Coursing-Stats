...
| `standingScore` | `(3×🥇+1×🥈+0.5×🥉) / (starts+4)` — при равных медалях выше меньше стартов |
...
После парса/правки результатов → `yarn run build-all-data`.  
Парсеры: см. [06-parsers](06-parsers.md).

## Pitfalls

- Не сортировать зачёт по Elo; не смешивать медали и CS в одно взвешенное число.
- «Чаще в призах» = выше КПД (меньше стартов при том же наборе медалей), не «больше стартов».
