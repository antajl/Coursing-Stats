...
| SPA fallback | CF Pages SPA mode (нет корневого `404.html`) → неизвестные URL отдают `/index.html`. **Нельзя** `/* /*.html 200` — pretty-URL даёт 308 на `/name`. `/spa-shell` и `/404` → 301 `/`. SEO URL — через prerender static HTML. |
