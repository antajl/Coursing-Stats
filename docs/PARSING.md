...
> **Полный workflow, заливка D1, ошибка `D1_RESET_DO`, батчи:** [`DATABASE.md`](DATABASE.md) — секция «Календарь и обновление D1»
...
- **Отдельной таблицы `judges` на remote нет** — имена агрегируются из `events.judges` и `results.raw_scores` (API `/api/judges`)
- Поле `judges` в таблице `events` (JSON или строка с текстом протокола)
- Поле `judges` в `results.raw_scores` — оценки по судьям в JSON забега
- Materialized-таблица `judge_stats` — в планах (`FUTURE-PLANS.md`)

---

## Рекорды Донино

Подробное описание двух дисциплин, правил «не смешивать» и одной собаки в обоих листах — **`docs/SPEED-RECORDS.md`** (раздел «Два вида замеров»).
