...
const NOTES =
  'Локально (yarn run dev) календари всегда видны. На проде — только если true. Переключение: scripts/show-calendar-*.bat / hide-calendar-*.bat, затем deploy-to-github.bat. Протоколы /event и /shows/exhibition этим флагом не открываются.'
...
Локально календари и так видны при yarn run dev.
`)
